...
def run_inference(
    ...
    croper = Croper('video_retalking/checkpoints/shape_predictor_68_face_landmarks.dat')
    full_frames_RGB = [cv2.cvtColor(frame, cv2.COLOR_BGR2RGB) for frame in full_frames]
    full_frames_RGB, crop_coords, quad = croper.crop(full_frames_RGB, xsize=512)
    ...
    if not os.path.isfile(coeffs_path) or args.exp_img is not None or args.re_preprocess:  # Changed: Use args.exp_img and args.re_preprocess
        net_recon = load_face3d_net(args.face3d_net_path, device)  # Changed: Use args.face3d_net_path
    ...
    D_Net, model = load_model(args, device)  # Changed: Pass args to load_model
